...
async function loadStats() {
  const [prodRes, ordRes, usersRes, bookRes, popRes, revRes, visRes] = await Promise.all([
    fetch(`${API}/products`),
  ...
  const visits   = visRes.ok   ? await visRes.json()    : [];
  ...
  const wVisits   = visits.filter(v => isThisWeek(v.created_at)).length;
  ...
  document.getElementById('stat-w-visits').textContent   = wVisits;
